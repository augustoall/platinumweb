...
session_start();
require_once '../../NFE-LIB/vendor/autoload.php';
require_once '../../includes/load__class__file_path_2.php';

$nfe = new NFePHP\NFe\Make();
$empresaConfig = new EmpresaConfigInstance();
$emp = new EmpresaConfigBean();

$emp = $empresaConfig->c_busca_empresa_config('22124254812');

//function taginfNFe($std):DOMElement
//Node principal
//Parametro	Tipo	Descrição
//stcClass	contêm os dados dos campos, nomeados conforme manual
$std = new stdClass();
...
//stcClass	contêm os dados dos campos, nomeados conforme manual
$std = new stdClass();
$std->cUF = $emp->getEmp_cUF();
$std->cNF = $emp->getEmp_cNF();
$std->natOp = $emp->getEmp_natOp();
$std->indPag = 0; //NÃO EXISTE MAIS NA VERSÃO 4.00 
$std->mod = $emp->getEmp_mod();
$std->serie = $emp->getEmp_serie();
$std->nNF = 2; // numero do docuemnto fiscal
$std->dhEmi = date("Y-m-d\TH:i:sP"); //date("Y-m-d\TH:i:sP");//Formato: “AAAA-MM-DDThh:mm:ssTZD” (UTC - Universal Coordinated Time).

if ($emp->getEmp_mod() == '55') {
    $std->dhSaiEnt = date("Y-m-d\TH:i:sP"); //Não informar este campo para a NFC-e.
}
if ($emp->getEmp_mod() == '65') {
    $std->dhSaiEnt = NULL; //Não informar este campo para a NFC-e.
}


$std->tpNF = $emp->getEmp_tpNF();
$std->idDest = $emp->getEmp_idDest();
$std->cMunFG = $emp->getEmp_cMunFG();
$std->tpImp = $emp->getEmp_tpImp();
$std->tpEmis = $emp->getEmp_tpEmis();
$std->cDV = 2;
$std->tpAmb = 2;
$std->finNFe = $emp->getEmp_finNFe();
$std->indFinal = $emp->getEmp_indFinal();
$std->indPres = $emp->getEmp_indPres();
$std->procEmi = $emp->getEmp_procEmi();
$std->verProc = $emp->getEmp_verProc();
$std->dhCont = null;
$std->xJust = null;
$elem = $nfe->tagide($std);
...
//stcClass	contêm os dados dos campos, nomeados conforme manual
$std = new stdClass();
$std->xNome = $emp->getEmp_nome(); //Razão Social ou Nome do emitente
$std->xFant = 'implementar'; //Nome fantasia
$std->IE = 'implementar';
$std->IEST = $emp->getEmp_incricao_est();
$std->IM = 'implementar';
$std->CNAE = 'implementar';
$std->CRT = 'implementar';
$std->CNPJ = $emp->getEmp_cnpj(); //indicar apenas um CNPJ ou CPF
$std->CPF = $emp->getEmp_cpf();
...
//stcClass	contêm os dados dos campos, nomeados conforme manual
$std = new stdClass();
$std->xLgr = $emp->getEmp_endereco();
$std->nro = $emp->getEmp_numero();
$std->xCpl = 'implementar';
$std->xBairro = $emp->getEmp_bairro();
$std->cMun = $emp->getEmp_cMunFG();
$std->xMun = $emp->getEmp_cidade();
$std->UF = 'implementar';
$std->CEP = $emp->getEmp_cep();
$std->cPais = 1058; //1058=Brasil
$std->xPais = 'Brasil';
$std->fone = $emp->getEmp_contato1(); //Preencher com o Código DDD + número do telefone. Nas operações com exterior é permitido informar o código do país + código da localidade + número do telefone (v2.0) 
...
//stcClass	contêm os dados dos campos, nomeados conforme manual
$std = new stdClass();
$std->xNome = 'implementar';
$std->indIEDest = 'implementar';
$std->IE = 'implementar';
$std->ISUF = 'implementar';
$std->IM = 'implementar';
$std->email = 'implementar';
$std->CNPJ = 'implementar'; //indicar apenas um CNPJ ou CPF ou idEstrangeiro
$std->CPF = 'implementar';
$std->idEstrangeiro = 'implementar';
...
// impressao de uma tag na tela
//$elem = $nfe->tagprod($elemtagprod);
//header('Content-type: text/xml; charset=UTF-8');
//echo $nfe->dom->saveXML($elem);